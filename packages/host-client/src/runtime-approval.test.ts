...
describe("runtime approval presentation helpers", () => {
  it("sorts approvals from newest to oldest update", () => {
    expect(
      sortRuntimeApprovalsForPresentation(approvals).map(
        (approval) => approval.approvalId
      )
    ).toEqual(["approval-newer", "approval-older"]);
  });

  it("filters approvals by status, session, conversation, requester, and approver", () => {
    expect(
      filterRuntimeApprovalsForPresentation(approvals, {
        status: "pending"
