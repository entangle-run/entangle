...
export function createHostClient(options: HostClientOptions) {
  ...
  return {
    ...
    async inspectHostEventIntegrity(): Promise<HostEventIntegrityResponse> {
      return parseResponse(
        await hostFetch(`${baseUrl}/v1/events/integrity`),
        hostEventIntegrityResponseSchema
      );
    },

    async exportSignedHostEventIntegrityReport(): Promise<HostEventIntegritySignedReportResponse> {
      return parseResponse(
    ...
    async publishRuntimeSourceHistory(
      nodeId: string,
      sourceHistoryId: string,
      request: RuntimeSourceHistoryPublishRequest = {}
    ): Promise<RuntimeSourceHistoryPublishResponse> {
      return parseResponse(
        await hostFetch(
          `${baseUrl}/v1/runtimes/${nodeId}/source-history/${sourceHistoryId}/publish`,
          {
            ...
            body: JSON.stringify(
              runtimeSourceHistoryPublishRequestSchema.parse(request)
            )
          }
        ),
        runtimeSourceHistoryPublishResponseSchema
    ...
    async publishRuntimeWikiRepository(
      nodeId: string,
      request: RuntimeWikiPublishRequest = {}
    ): Promise<RuntimeWikiPublishResponse> {
      return parseResponse(
        await hostFetch(
          `${baseUrl}/v1/runtimes/${nodeId}/wiki-repository/publish`,
          {
            ...
            body: JSON.stringify(runtimeWikiPublishRequestSchema.parse(request))
          }
        ),
        runtimeWikiPublishResponseSchema
      );
    ...
    async upsertRuntimeWikiPages(
      nodeId: string,
      request: RuntimeWikiUpsertPageBatchRequest
    ): Promise<RuntimeWikiUpsertPageBatchResponse> {
      return parseResponse(
        await hostFetch(
          `${baseUrl}/v1/runtimes/${nodeId}/wiki/pages/batch`,
          {
            method: "POST",
            headers: {
              "content-type": "application/json"
            ...
            body: JSON.stringify(
              runtimeWikiUpsertPageBatchRequestSchema.parse(request)
            )
          }
        ),
        runtimeWikiUpsertPageBatchResponseSchema
