...
| --- | --- | --- | --- |
| Contracts and schemas | Package, graph, runtime context, per-node agent runtime selection, A2A, artifacts, sessions, approvals, turns, generic engine-session-id/version outcomes, generic engine permission observations, generic agent-runtime inspection DTOs, workspace-health inspection DTOs, host DTOs. | Some deeper diagnostics, resumable OpenCode approval DTOs, and future API generation remain later work. | Production tenancy contracts. |
| Host control plane | Local persistent state, resource mutation, runtime lifecycle, events, recovery, session and runtime inspection, including effective agent-runtime status summaries and workspace-health summaries. | `DELETE /v1/runtimes/{nodeId}` remains absent from the broader spec. | Production RBAC/ABAC and multi-tenant auth. |
...
- Per-node agent runtime contracts now exist and default to OpenCode, and the
  ...
  engine-state/wiki layout. It still lacks the complete policy bridge,
  resumable approval mapping, external cancellation, artifact harvesting,
