...
- safe restore for supported git-backed runtime artifacts, producing durable
  restore-attempt records and materializing git tree contents into explicit
  artifact workspace restore directories without overwriting existing targets
  by default.
...
- Per-node agent runtime contracts now exist and default to OpenCode, and the
  ...
  explicit artifact workspace restore directories through the host and CLI
  without overwriting existing targets by default.
