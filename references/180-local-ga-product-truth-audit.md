...
- Per-node agent runtime contracts now exist and default to OpenCode, and the
  ...
  commit artifact publication with node-policy approval gates, and bounded
  artifact history/diff inspection. Entangle Local still lacks the complete
  policy bridge, resumable approval mapping, operation-scoped approval evidence,
  external cancellation, non-primary publication provisioning/fallback,
