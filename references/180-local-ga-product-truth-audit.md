...
- OpenCode text outcomes can now carry bounded `entangle-actions` blocks that
  are validated into generic handoff directives before Entangle performs any
...
- Per-node agent runtime contracts now exist and default to OpenCode, and the
  ...
  provisioning/fallback, artifact restore/replay workflow, doctor remediation,
  and richer runtime evidence panels required for L3 acceptance.
