...
- host-written external session cancellation requests, host-client, CLI, and
  Studio cancellation surfaces, runner observation while idle or mid-turn,
  engine `AbortSignal` propagation, OpenCode child-process termination on
  cancellation, and cancelled session/turn lifecycle evidence.
...
- bounded engine-request summary evidence on executable turns, exposing prompt
  part counts, aggregate prompt size, memory, artifact, and tool counts,
  execution limits, and peer-route inclusion through runner turns, host
  activity/events, CLI, and Studio without storing raw prompt text or
  runtime-local paths.
- executable engine turn assembly now explicitly includes agent-runtime,
...
- OpenCode text outcomes can now carry bounded `entangle-actions` blocks that
  are validated into generic handoff directives and policy-scoped approval
  request directives before Entangle performs any peer-message side effect or
  records any approval gate.
- Unauthorized or unroutable handoff directives now fail as `policy_denied`
  while preserving bounded engine evidence on the failed runner turn.
- Engine-requested approval directives now materialize pending runner approval
  records with operation/resource evidence, link the approval ids to the
  runner turn, and move the active session/conversation to
  `waiting_approval`/`awaiting_approval` without granting the gated side
  effect or publishing a task result.
...
- source-history publication for applied source-history records, producing
  ...
  default, provisioning selected non-primary `gitea_api` publication targets
  before the git push path, emitting `source_history.published`, and
  exercising local `file://` git remotes through real git pushes in host
  coverage.
- safe restore for supported git-backed runtime artifacts through host, CLI,
  and Studio, producing durable restore-attempt records and materializing git
  tree contents into explicit artifact workspace restore directories without
  overwriting existing targets by default.
- approval-gated artifact promotion from successful restore workspaces into the
  node source workspace through host, shared host-client, CLI, and Studio
  surfaces, with exact `source_application` approval scope on the
  artifact/restore tuple.
...
- host-scoped runtime approval decisions can now create exact
  operation/resource-scoped approval records for source mutation workflows or
  decide existing pending approvals, with shared host-client, CLI, Studio, and
  `approval.trace.event` coverage.
...
- Local has reset-by-volume-teardown, a first read-only doctor diagnostic,
  first versioned backup/validated restore commands, and a first conservative
  repair command, but no productized upgrade command yet.
- Local has CLI and Studio session launch over the host boundary, CLI wait
  ...
  Relay-publish retry, graph bundles, broader artifact replay/promotion beyond
  the first source-workspace path, and the full OpenCode-backed agentic-node
  runtime are not complete.
- Per-node agent runtime contracts now exist and default to OpenCode, and the
  ...
  artifact history/diff inspection, with CLI and Studio graph-backed
  agent-runtime configuration now available for node runtime mode, engine
  profile, and default-agent overrides. The runner now also commits local
  `memory/wiki` snapshots into the node `wiki-repository` after completed
  turns, exposes sync outcomes through turn inspection, and records bounded
  engine-request summaries for executable turns without raw prompt storage,
  while executable prompt assembly carries agent-runtime, workspace-boundary,
  policy, inbound-control, and Entangle action-contract context, and OpenCode
  can return bounded `entangle-actions` blocks that Entangle validates into
