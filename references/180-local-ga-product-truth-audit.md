...
- Local has CLI and Studio session launch over the host boundary, CLI wait
  ...
  Relay-publish retry, graph bundles, approval-gated artifact replay/promotion,
  and the full OpenCode-backed agentic-node runtime are not complete.
- Per-node agent runtime contracts now exist and default to OpenCode, and the
  ...
  approval mapping, external cancellation, non-primary publication
  provisioning/fallback, approval-gated artifact replay/promotion workflow,
