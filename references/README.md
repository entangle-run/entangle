...
memory.
Agent engine turn prompts now also carry conversation id, turn id, optional
parent message id, and from/to node ids inside the bounded inbound-control
context, giving each coding engine message-routing context while Entangle keeps
authority over routing and side effects.
...
Distributed proof profiles, generated verifier commands, and proof kit
generation can now also require URL-backed default agent engine profiles to be
non-loopback and non-wildcard with
`--require-external-agent-engine-urls`, keeping physical attached OpenCode and
`external_http` proof runs from accepting local-only engine endpoints.
...
They can also require external URL-backed agent engine profiles, rejecting a
loopback or wildcard default engine `baseUrl` without probing the endpoint.
...
missing-git-service rejection, loopback-git-service URL rejection, and
loopback-agent-engine URL rejection.
