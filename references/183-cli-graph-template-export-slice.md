...
- Registered the existing canonical `examples/federated-preview/graph.json` as the
  built-in `federated-preview` graph template.
...
pnpm --filter @entangle/cli dev graph templates export federated-preview <tmp>/graph.json
