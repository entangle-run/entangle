...
- a widening of the host event surface where `entangle-host` now derives
  ...
  active-conversation ids, root artifact ids, and last message type so runtime
  traces can distinguish active work from session history, while persisting
  those observations under observed host state and emitting them only when the
  durable observed fingerprint changes;
