...
function createAgentEngineForRuntimeContext(input: {
  ...
  if (agentRuntime.engineProfile.kind === "external_http") {
    return createExternalHttpAgentEngine({
      runtimeContext
    });
  }
      ...
      (async ({ assignment, runtimeContextPath }) => {
        ...
        return {
          ...
          reconcileSourceHistory: (request) =>
            configured.service.requestSourceHistoryReconcile(request),
          ...
          upsertWikiPage: (request) =>
            configured.service.requestWikiPageUpsert(request),
          patchWikiPages: (request) =>
            configured.service.requestWikiPatchSet(request),
